...
class Usdc extends _i1.GeneratedContract {
  ...
  Future<String> init(
    String _domainName,
    String _domainVersion, {
    required _i1.Credentials credentials,
    _i1.Transaction? transaction,
  }) async {
    final function = self.abi.functions[18];
    assert(checkSignature(function, '7029144c'));
    final params = [
      _domainName,
      _domainVersion,
    ];
  ...
  Future<String> initialize(
    ...
    assert(checkSignature(function, '4cd88b76'));
    final params = [
      name,
      symbol,
    ];
  ...
  /// set, the function will be evaluated in the specified block. By default, the
  /// latest on-chain block will be used.
  Future<_i1.EthereumAddress> proxyAdmin({_i1.BlockNum? atBlock}) async {
    final function = self.abi.functions[27];
    assert(checkSignature(function, '3e47158c'));
    final params = [];
    final response = await read(
      function,
      params,
  ...
  /// like the gas price, nonce and max gas. The `data` and `to` fields will be
  /// set by the contract.
  Future<String> transferProxyAdmin(
    _i1.EthereumAddress _newProxyAdmin, {
    required _i1.Credentials credentials,
    _i1.Transaction? transaction,
  }) async {
    final function = self.abi.functions[35];
    assert(checkSignature(function, '8356ca4f'));
    final params = [_newProxyAdmin];
    return write(
      credentials,
      transaction,
      function,
